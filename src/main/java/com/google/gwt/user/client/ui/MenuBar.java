...
public class MenuBar extends Widget implements PopupListener, HasAnimation, HasCloseHandlers<PopupPanel> {
	 ...
	 * 
	 * Notice that this interface is package protected. This interface need not be publicly exposed, as it is
	 * only used by the MenuBar class to provide RTL versions of the images in the case the the user does not
	 * pass in their own bundle. However, we cannot make this class private, because the generated class needs
	 * to be able to extend this class.
	 */
	interface MenuBarImagesRTL extends MenuBarImages {

		/**
		 * An image indicating a {@link MenuItem} has an associated submenu for a RTL context.
		 * 
		 * @return a prototype of this image
		 */
	...
	void selectItem(MenuItem item) {
		...
		if (item != null) {
			...
			Accessibility.setState(getElement(), Accessibility.STATE_ACTIVEDESCENDANT, DOM.getElementAttribute(item
					.getElement(), "id"));
		}
	...
	/**
	 * Selects the first item in the menu if no items are currently selected. This method assumes that the
	 * menu has at least 1 item.
	 * 
	 * @return true if no item was previously selected and the first item in the list was selected, false
	 *         otherwise
	 */
