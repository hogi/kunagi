...
import gwtupload.server.MemoryUploadListener;
 ...
 * 
 * <ul>
 * <li>For customizable application actions, it's better to extend the UloadAction class instead of this.</li>
 * 
 * <li>
 * This servlet supports to be deployed in google application engine. It is able to detect this environment
 ...
 * <li>Uses memcache for session tracking instead of normal session objects, because objects stored in session
 * seem not to be available until the thread finishes</li>
 * </ul>
 * </li>
 * </ul>
 * 
 ...
 * 
 */
public class FileUploadServletServlet extends HttpServlet implements Servlet {
	...
	/**
	 * Utility method to get a fileItem of type file from a vector using either the file name or the attribute
	 * name.
	 * 
	 * @param sessionFiles
	 * @param parameter
	...
	public static FileItem findItemByFieldName(List<FileItem> sessionFiles, String attrName) {
		if (sessionFiles != null) {
			for (FileItem fileItem : sessionFiles) {
				if (fileItem.getFieldName().equalsIgnoreCase(attrName)) { return fileItem; }
			}
		}
		return null;
	}
	...
	/**
	 * Utility method to get a fileItem from a vector using the file name It only returns items of type file.
	 * 
	 * @param sessionFiles
	 * @param fileName
	 * @return fileItem of the file found or null
	...
	public static FileItem findItemByFileName(List<FileItem> sessionFiles, String fileName) {
		if (sessionFiles != null) {
			for (FileItem fileItem : sessionFiles) {
				if (fileItem.isFormField() == false && fileItem.getName().equalsIgnoreCase(fileName)) { return fileItem; }
			}
		}
		return null;
	}
	...
	public static String getMessage(String key, Object... pars) {
		ResourceBundle res = ResourceBundle.getBundle(UploadServlet.class.getName(), getThreadLocalRequest()
				.getLocale());
	...
	/**
	 * Removes all FileItems stored in session, but in this case the user can specify whether the temporary
	 * data is removed from disk.
	 * 
	 * @param request
	 * @param removeData true: the file data is deleted. false: use it when you are referencing file items
	...
	/**
	 * Writes a XML response to the client. The message must be a text which will be wrapped in an XML
	 * structure.
	 * 
	 * Note: if the request is a POST, the response should set the content type to text/html or text/plain in
	 * order to be able in the client side to read the iframe body (submitCompletEvent.getResults()),
	 ...
	 * @throws IOException
	 */
	protected static void renderXmlResponse(HttpServletRequest request, HttpServletResponse response, String message,
	...
	public void checkRequest(HttpServletRequest request) {
		...
		if (request.getContentLength() > maxSize) { throw new UploadSizeLimitException(maxSize,
	...
	public void getUploadedFile(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (item != null) {
			logger.debug("UPLOAD-SERVLET (" + request.getSession().getId() + ") getUploadedFile: " + parameter
					+ " returning: " + item.getContentType() + ", " + item.getName() + ", " + item.getSize() + " bytes");
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if (request.getParameter(PARAM_SESSION) != null) {
			...
			renderXmlResponse(request, response, "<" + TAG_BLOBSTORE_PATH + ">" + getBlobstorePath(request) + "</"
					+ TAG_BLOBSTORE_PATH + ">");
	...
	/**
	 * The post method is used to receive the file and save it in the user session. It returns a very XML page
	 * that the client receives in an iframe.
	 * 
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException,
		...
		String error;
		try {
			Log.DEBUG("********** parsePostRequest()-in");
			error = parsePostRequest(request, response);
			Log.DEBUG("********** parsePostRequest()-out finish()-in");
			finish(request);
			Log.DEBUG("********** finish()-out renderXmlResponse()-in");
			renderXmlResponse(request, response, error != null && error.length() > 0 ? "<error>" + error + "</error>"
					: FINISHED_OK, true);
			Log.DEBUG("********** finish()-out renderXmlResponse()-out");
	...
	/**
	 * Method executed each time the client asks the server for the progress status. It uses the listener to
	 * generate the adequate response
	 * 
	 * @param request
	 * @param fieldname
		...
		if (listener != null) {
			if (listener.getException() != null) {
				if (listener.getException() instanceof UploadCanceledException) {
					...
					logger.error("UPLOAD-SERVLET (" + session.getId() + ") getUploadStatus: " + fieldname
							+ " cancelled by the user after " + listener.getBytesRead() + " Bytes");
				} else {
			...
			if (fieldname == null) {
				...
				logger.debug("UPLOAD-SERVLET (" + session.getId() + ") getUploadStatus: " + request.getQueryString()
						+ " finished with files: " + session.getAttribute(ATTR_FILES));
			} else {
	...
	/**
	 * This method parses the submit action, puts in session a listener where the progress status is updated,
	 * and eventually stores the received data in the user session.
	 * 
